...
public class DicoRepository
{
    ...
    public static ReportViewModel GenerateReport() => new ReportViewModel
    {
        ...
        ArticlesWithDalletNamesCount = Doc.Articles.Where(x => !x.IsRedirected && x.StandardizedFormOf is null && x.DalletNames.Any()).Count(),
        ArticlesWithoutDalletNamesCount = Doc.Articles.Where(x => !x.IsRedirected && x.StandardizedFormOf is null && !x.DalletNames.Any()).Count(),
        StandardizedFormsCount = Doc.Articles.Where(x => x.StandardizedFormOf is not null).Count(),
